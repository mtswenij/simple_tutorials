...
public class AccountNotifier extends Job {
	...
	public void doJob() {
		...
		if(!isLocked()) {  // if there is already an account notifier running, don't run again
			...
			// fetch at most 25 AccountNotifications ordered by timestamp ascending
			List<Notification> bids2Notify = Model.all(Notification.class)
					.filter("type", Notification.ACCOUNT).order("timeStamp")
					.fetch(25);
	...
	public List<String> generateDeviceList(Notification n) {
		...
		// get account
		Account act = Model.all(Account.class).getByKey(n.payLoadID);
